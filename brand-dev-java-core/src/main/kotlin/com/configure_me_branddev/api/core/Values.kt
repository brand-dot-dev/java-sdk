...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { BrandDevInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw BrandDevInvalidDataException("`$name` is not set")
            is JsonNull -> throw BrandDevInvalidDataException("`$name` is null")
            else -> throw BrandDevInvalidDataException("`$name` is invalid, received $this")
        }
